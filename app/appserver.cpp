...

		{
...

					{
						...
						case 0: 

							{
								...
								taskQueue.push(new data_buf_t[1]);

								taskQueue.back()->id = 1;

								taskQueue.back()->cliFD = udtbuf.cliFD;

								taskQueue.back()->srvFD = ttt;

								strcpy(taskQueue.back()->buf, "ack");

								taskQueue.back()->len = sizeof(taskQueue.back()->buf);



								cout << "\tudtQueue.back().id:"<<taskQueue.back()->id << endl;

								cout << "\tudtQueue.back().cliFD:"<< taskQueue.back()->cliFD << endl;

								cout << "\tudtQueue.back().srvFD:"<< taskQueue.back()->srvFD << endl;

								cout << "\tudtQueue.back().len:"<< taskQueue.back()->len << endl;
						...
						case 2:

							{
...

								{
									...
									taskQueue.back()->srvFD = udtbuf.srvFD;

									memcpy(taskQueue.back()->buf, udtbuf.buf, udtbuf.len);

									taskQueue.back()->len = udtbuf.len;

									

									printf("\t[S->S*] data transfer.\n");

									cout << "\ttcpQueue.back().id:"<<taskQueue.back()->id << endl;

									cout << "\ttcpQueue.back().cliFD:"<< taskQueue.back()->cliFD << endl;

									cout << "\ttcpQueue.back().srvFD:"<< taskQueue.back()->srvFD << endl;

									cout << "\ttcpQueue.back().len:"<< taskQueue.back()->len << endl;
						...
						case 5:
							...
							printf("\t[C->S] Recv connect ack ready.\n");
							cout << "\tudtbuf.cliFD:" << udtbuf.cliFD << "\tudtbuf.srvFD:" << udtbuf.srvFD << endl;
							map_insert(&connectMap, udtbuf.srvFD, udtbuf.cliFD);
							UDT::epoll_add_ssock(eid, udtbuf.srvFD);
...

			{
...

				{
...

					{
						...
						taskQueue.back()->cliFD = connectMap_iter->second;

						taskQueue.back()->srvFD = *i;

						memset(taskQueue.back()->buf, 0, UDT_DATA_MAX_LEN);

						taskQueue.back()->len = 0;
...

				{
...

					{
						...
						memcpy(taskQueue.back()->buf, tcpdata, rs);

						taskQueue.back()->len = rs;
			...
			if (!taskQueue.empty())
			{
				...
				case 4:
				{
					...
					if (UDT::ERROR == (rs = UDT::send(client, (char*)(taskQueue.front()), udtsize, 0)))
				...
				case 7:
				{
					...
					cout << "\ttcpQueue.back().id:" << taskQueue.front()->id << endl;
					cout << "\ttcpQueue.back().cliFD:" << taskQueue.front()->cliFD << endl;
					cout << "\ttcpQueue.back().srvFD:" << taskQueue.front()->srvFD << endl;
					cout << "\ttcpQueue.back().len:" << taskQueue.front()->len << endl;

					int rs = send(taskQueue.front()->srvFD, taskQueue.front()->buf, taskQueue.front()->len, 0);
					if (0 > rs)
					{
						cout << "error1.\n";
						taskQueue.pop();
